...
def download_summary(Config : Configuration) -> None:
    ...
    count_total = count_normal + count_type_1 + count_type_2 + count_type_3 + count_type_4

    col1.image(get_image('anomaly_type_01.png'), use_column_width = 'auto')
    col1.metric('Normal observations', int(count_normal), f'{(count_normal/count_total)*100:.2f}%', delta_color='normal')
    col2.image(get_image('anomaly_type_01.png'), use_column_width = 'auto')
    col2.metric('Anomalies of Type 1', int(count_type_1), f'{(count_type_1/count_total)*100:.2f}%', delta_color='inverse')
    col3.image(get_image('anomaly_type_02.png'), use_column_width = 'auto')
    col3.metric('Anomalies of Type 2', int(count_type_2), f'{(count_type_2/count_total)*100:.2f}%', delta_color='inverse')
    col4.image(get_image('anomaly_type_03.png'), use_column_width = 'auto')
    col4.metric('Anomalies of Type 3', int(count_type_3), f'{(count_type_3/count_total)*100:.2f}%', delta_color='inverse')
    col5.image(get_image('anomaly_type_04.png'), use_column_width = 'auto')
    col5.metric('Anomalies of Type 4', int(count_type_4), f'{(count_type_4/count_total)*100:.2f}%', delta_color='inverse')
...
def generate_data(Config : Configuration):
    ...
    amounts_listed = [count_normal, count_type_1, count_type_2, count_type_3, count_type_4]
    ...
    with st.spinner(f'Calculating paths for {amounts_counted} screw runs...'):
        ...
        for idx in range(amounts_counted): 
            ...
            # get obs type
            obs_types = ['ok', 'anomaly_type_1', 'anomaly_type_2', 'anomaly_type_3', 'anomaly_type_4']
            # loop ofer obs types
            for i in range(5):
