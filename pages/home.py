...
def home() -> None:
    ...
    st.write('The generator "Screw Data" provides an easy way to generate screw-driving data. It uses a simple linear baseline\
...
def home_sb() -> None:
    ...
    with st.sidebar.expander('Parameter Selection'):
        st.write('The tool is designed in such a way that all parameter entries can be made in the side menu. No entries can\
    ...
    with st.sidebar.expander('Parameter storage'):
        st.write('As you navigate the tool, the parameter will be saved. At download, they are used to generate the data set.')

    with st.sidebar.expander('Parameter import'):
        st.write('You can import parameter from previous usage in the following section')
