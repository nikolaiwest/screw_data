...
from utils.configuration import Configuration, get_distribution, bool_to_numeric, show_linear_basis, show_linear_basis_observation, update_config
...
def baseline_sb(Config : Configuration, parameter : dict, info_text : dict) -> None:
    ...
    with st.sidebar.expander('Adjust the Linear Basis used in all Observations:'):
    ...
    with st.sidebar.expander('Adjust the Random Scattering to apply Noise:'): 
        baseline_sb_random_scattering(Config, parameter, info_text)
...
def baseline_sb_linear_basis(Config : Configuration, parameter : dict, info_text : dict) -> None:
    ...
    if col1.button('Add a new Point'):
        ...
        Config.y_steps[f'y{new_idx}'] = Config.y_steps.get(f'y{new_idx-1}') + 1
    # remove last P(xi, yi)    
    if col2.button('Remove last Point'): # TODO implement function...
        last_idx = len(Config.x_steps)-1
        Config.x_steps.pop(f'x{last_idx}')
        Config.y_steps.pop(f'y{last_idx}')

    # numeric input for all P(xi, yi)
    for idx in range(Config.init_len):
...
def baseline_sb_smoothing_parameter(
    ...
    Config.selected_type = st.selectbox('Select smoothing method', Config.types, index=0)
    # filter parameter according to selction
    if Config.selected_type == Config.types[0]: #'Savitzky-Golay filter'
        Config.filter_sg_window_length = st.slider(
            label = 'Adjust the Length of the Smoothing Window',
            min_value = parameter['filter']['filter_sg_window_length_min'],
            max_value = parameter['filter']['filter_sg_window_length_max'],
            value = parameter['filter']['filter_sg_window_length_val'],
            step = parameter['filter']['filter_sg_window_length_stp'],
            help = info_text['filter']['filter_sg_window_length_tt'])
        Config.filter_sg_poly_order = st.slider(
            label = 'Adjust the Polynomial Order',
            min_value = parameter['filter']['filter_sg_poly_order_min'],
            max_value = parameter['filter']['filter_sg_poly_order_max'],
            value = parameter['filter']['filter_sg_poly_order_val'],
            step = parameter['filter']['filter_sg_poly_order_stp'],
            help = info_text['filter']['filter_sg_poly_order_tt'])
    if Config.selected_type == Config.types[1]: #'No filter'
        pass 

def baseline_sb_random_scattering(
    Config : Configuration, 
    parameter : dict, 
    info_text : dict,
    ) -> None:
    ...
    '''
    # Checkbox 1
    Config.rnd_scattering_apply = bool_to_numeric(st.checkbox(
    ...
    if Config.rnd_scattering_apply:
        ...
        Config.rnd_scattering_weibull_alpha) = get_distribution(
    ...
    # Checkbox 2
    Config.rnd_tightening_apply = bool_to_numeric(st.checkbox(
    ...
    if Config.rnd_tightening_apply:
        ...
        Config.rnd_tightening_weibull_alpha) = get_distribution(
...
def remove_neg_y_values(Config : Configuration) -> None:
    ...
    Config.remove_neg_y_values = bool_to_numeric(st.sidebar.checkbox(
        label = 'Set negative y-values to zero',
        value = Config.remove_neg_y_values))
...
def baseline(Baseline : Observation, Config : Configuration, info_text : dict) -> None:
    ...
    st.header('Generation of a Baseline that Determines the General Shape of all Observations')
    ...
    with st.expander('Baseline: Further Explanation'):
        st.write('The baseline represents the starting point for generating new observations of screw runs. Both the **normal** observations and the **anormal** \
        ...
        st.write('- In order to realize a high adaptability of the screw runs, all observations are based on a few points that are linearly interpolated. \
        ...
        st.write('- In order to realize a natural screw curve, a filter can then be applied to smooth the curve.')
        st.write('')

        st.write('**Adjust the Random Scattering to apply Noise**')
        st.write('Finally, two types of scattering can be inserted.')
        st.write('- On the one hand, a general noise can be applied to the entire observation.')
        st.write('- On the other hand, additional scattering can be generated in the tightening phase.')
        st.write('')
