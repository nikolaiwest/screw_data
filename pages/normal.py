...
def normal_sb(Config : Configuration, parameter : dict, info_text : dict) -> None:  
    ...
    with st.sidebar.expander('Adjust the horizontal offset to randomize observations: ⇔'): 
...
def normal_sb_horizontal(
    ...
    '''
    # Checkbox
    Config.offset_hori_apply = bool_to_numeric(st.checkbox(
...
def normal_sb_vertical(
    ...
    '''
    # Checkbox
    Config.offset_vert_apply = bool_to_numeric(st.checkbox(
        label = 'Apply random vertical offset to selected points',
        value = 1))
    # adjust parameter to specify the offset 
    if Config.offset_vert_apply:
        ...
        Config.offset_vert_weibull_alpha) = get_distribution(
...
def normal(observations : 'list[Observation]', Config : Configuration, info_text : dict) -> None:
    ...
    st.subheader('Visualization of a subset of screw-runs according to the selected Configuration')
