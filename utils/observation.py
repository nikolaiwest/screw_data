...
class Observation():
    ...
    def get_y(self, Config : Configuration) -> list: 
        ...
        # add a filter to smoothen the interpolation
    ...
    def apply_smoothing_filter(self, y_values : list, Config : Configuration) -> list:
        '''Applies a Savitzky-Golay filter to smoothen the data according to <window_length> and <poly_order>'''
    ...
    def apply_convolve_filter(self, y_values : list, Config : Configuration) ->list:
        import numpy as np
        y_values = y_values + [y_values[-1] for _ in range(25)]
        box_pts = 25
        box = np.ones(box_pts)/box_pts
        return np.convolve(y_values, box, mode='same')[:-25]

    def apply_rnd_scattering(self, 
        y_values : list, 
        Config : Configuration
        ) -> list:
        # apply random scattering to the entire observation
        ...
        if Config.rnd_scattering_selected_type=='Weibull':
            return [y + weibullvariate(Config.rnd_scattering_weibull_alpha, beta=1.0) for y in y_values]

    def apply_rnd_tighteining(self, 
        y_values : list, 
        Config : Configuration
        ) -> list:
        ...
        if Config.rnd_tightening_selected_type=='Normal':
            return [y + gauss(0, Config.rnd_tightening_normal_stadev) if (lower<x<upper) else y for x, y in zip(range(len(y_values)), y_values)]
        if Config.rnd_tightening_selected_type=='Uniform':
            uniform_range = Config.rnd_tightening_uniform_range/2
            return [y + uniform(-uniform_range, uniform_range) if (lower<x<upper) else y for x, y in zip(range(len(y_values)), y_values)]
        if Config.rnd_tightening_selected_type=='Weibull':
            return [y + weibullvariate(Config.rnd_tightening_weibull_alpha, beta=1.0) if (lower<x<upper) else y for x, y in zip(range(len(y_values)), y_values)]
        ...
        # apply vertical offset
            ...
            if Config.offset_vert_selected_type=='Normal':
                for point in Config.offset_vert_selected_points:
                    y_steps[f'y{point[-1:]}'] = float(y_steps[(f'y{point[-1:]}')] + gauss(0, Config.offset_vert_normal_stadev))
            if Config.offset_vert_selected_type=='Uniform':
                for point in Config.offset_vert_selected_points:
                    uniform_range = Config.offset_vert_uniform_range/2
                    y_steps[f'y{point[-1:]}'] = float(y_steps[(f'y{point[-1:]}')] + uniform(-uniform_range, uniform_range))
            if Config.offset_vert_selected_type=='Weibull':
                for point in Config.offset_vert_selected_points:
                    y_steps[f'y{point[-1:]}'] = float(y_steps[(f'y{point[-1:]}')] + weibullvariate(Config.offset_vert_weibull_alpha, beta=1.0))        
            return y_steps
    ...
    def apply_type_1_anomaly(self, Config : Configuration) -> None:
        ...
        if Config.anomaly_type_1_width_selected_type == 'Normal':
            width = int(Config.anomaly_type_1_width + gauss(0, Config.anomaly_type_1_width_normal_stadev))
        if Config.anomaly_type_1_width_selected_type == 'Uniform':
            uniform_range = Config.anomaly_type_1_width_uniform_range/2
            width = int(Config.anomaly_type_1_width + uniform(-uniform_range, uniform_range))
        if Config.anomaly_type_1_width_selected_type == 'Weibull':
            width = int(Config.anomaly_type_1_width + weibullvariate(Config.anomaly_type_1_width_weibull_alpha, beta=1.0))
        # get hight for this observation
        if Config.anomaly_type_1_hight_selected_type == 'Normal':
            hight = Config.anomaly_type_1_hight + gauss(0, Config.anomaly_type_1_hight_normal_stadev)
        ...
        if Config.anomaly_type_1_hight_selected_type == 'Weibull':
            hight = Config.anomaly_type_1_hight + weibullvariate(Config.anomaly_type_1_hight_weibull_alpha, beta=1.0)
        ...
        if Config.anomaly_type_1_leap_shape == 'Right-skewed':
            self.x_steps['x3'] = self.x_steps['x2'] + uniform(1, width/2-1,)
